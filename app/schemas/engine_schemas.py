"""Pydantic schemas that mirror the Engine API contracts.

These are used by the Backend to validate requests forwarded to the Engine
and to parse/validate Engine responses before returning them to clients.
They intentionally replicate the Engine's own schemas so the Backend has
no runtime dependency on the Engine package.
"""
...
class CreateSimulationRequest(BaseModel):
    """Mirrors Engine's CreateSimulationRequest exactly."""
...
class SimulationRecordResponse(BaseModel):
    """Mirrors Engine's SimulationRecordResponse."""
...
class SimulationStepResponse(BaseModel):
    """Mirrors Engine's SimulationStepResponse."""
